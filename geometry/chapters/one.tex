...
\begin{definition}
  A conic section, or a conic is a curve obtained from a cone's surface intersecting a plane.
\end{definition}

Germinal Pierre Dendelin, a 19th century French-Belgian Mathematician, discovered this proof
to demonstrate that any plane that cuts through a right circular cone produces a quadratic
curve.
...
Proof from \cite{brannan}, with some modifications.
...
\begin{proof}
  The line through $O$ parallel to $l_{PQ}$ necessarily meets $C$
  again, (counting algebraic multiplicities) since for any quadratic equation with real 
  coeffecients, if one of the roots is real, the other root must be real. Hence, $\oplus$ is
  closed over $C$.

  The point $O$ serves as the identity element, and when $Q$ is such that the
  line parellel to $l_{PQ}$ that passes through $O$ is tangent to the conic, i.e when $R=O$,
  we get $P\oplus Q=O$. Such $Q$ is the inverse of $P$.

  To prove associativity, we'll derive expressions for $P\oplus Q$ by paremeterization
  for standard conics, i.e for the circle $x^2+y^2=1$, for the parabloa $y=x^2$, and for the
  hyperbola $xy=1$. In the next Chapter, we will prove that all ellipses, hyperbolae, and
  parabolae are affine congruent to their respective standard forms. This result will
    ...
    \item
        ...
        \begin{figure}[H]
          ...
          \begin{tikzpicture}
            ...
            \tkzDrawLine[<->, line width=1, add=0.2 and 0.2, cyan](P,Q)
            \tkzDrawLine[<->, line width=1, add=0.2 and 0.2, cyan](O,R)
    ...
    \item
        ...
        \begin{figure}[H]
          ...
          \begin{tikzpicture}
            ...
            \tkzDrawLine[<->,line width=0.5, gray](x,x')
            \tkzDrawLine[<->, line width=1, add=0.2 and 0.2, cyan](P,Q)
            \tkzDrawLine[<->, line width=1, add=0.5 and 0.5, cyan](O,R)
    ...
    \item
        ...
        \begin{figure}[H]
          ...
          \begin{tikzpicture}
            ...
            \tkzDrawLine[<->,line width=0.5, gray](y,y')
            \tkzDrawLine[<->,line width=0.5, gray](x,x')
            \tkzDrawLine[<->, line width=1, add=0.4 and 0.6, cyan](P,Q)
            \tkzDrawLine[<->, line width=1, add=0.4 and 0.2, cyan](O,R)
        ...
        \end{figure}
  \end{enumerate}
  From the expressions obtained for $P\oplus Q$, in all three cases,\\
  $P\oplus(Q\oplus R)=(P\oplus Q)\oplus R$
\end{proof}

Proof inspired from \cite{shirali}.
