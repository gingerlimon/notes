...
\begin{remark}
  ...
  If dim $E=1$, $E$ itself is a line, and thus the set of linea comtains a unique elemmaent,
  $P(E)$ is a point.
\end{remark}
...
\begin{proof}
  ...
  We can use the linear algebraic propositionerties to further deduce that:
  \[
  ...
  Now, to prove the second propositionerty, let $J$ be the vector hyperplane of which $H$ is image of.
...
\section{Homogeneous Coordinates and Projective Frames}
...
\begin{lemma}
  ...
  $p(e_1+\cdots+e_{n+1})=p(e'_1+\cdots+e'_{n+1})=m_0$, then they are propositionortional.
\end{lemma}
...
\begin{proof}
  ...
  \]
  As we are dealing with a basis, $\lambda_i=\lambda$. Thus two bases are propositionortional.
\end{proof}

\section{Some Results in Projective Geometry}

\begin{theorem}[Fundamental Theorem of Projective Geometry]
  \label{theorem:fundprojgeo}
...
\begin{proof}
  ...
  projrctive propositionerties, to deduce that the theorem holds in general.
...
\begin{proposition}
  If $a$, $b$, $c$, and $d$ are four collinear distinct points, then the following equalities
  hold
  \begin{eqnarray*}
